# -*- coding: utf-8 -*-
"""
Created on Mon Jul 29 16:58:26 2024
...
# Enlever les genes d'AEGIS qu'il n'y a pas dans CRUKPAP (dans AEGIS il n'y a pas de colonne patientid, on fusionne après sur la base des noms des lignes)
...
# Obtenir les noms de colonnes de df_genes_CRUKPAP
colonnes_CRUKPAP = df_genes_CRUKPAP.columns

# Filtrer les colonnes de df_genes_AEGIS pour ne garder que celles présentes dans df_genes_CRUKPAP
df_genes_AEGIS = df_genes_AEGIS.loc[:, df_genes_AEGIS.columns.isin(colonnes_CRUKPAP)]

# Sélectionner les colonnes souhaitées de df_clinique_AEGIS
clinique_cols = ['sex', 'age', 'smoking_status', 'packyears']
df_clinique_filtered = df_clinique_AEGIS[clinique_cols].copy()

# Remplacer les valeurs dans la colonne 'sex'
df_clinique_filtered['sex'] = df_clinique_filtered['sex'].replace({'Male': 1, 'Female': 2})

# Remplacer les valeurs dans la colonne 'smoking_status'
df_clinique_filtered['smoking_status'] = df_clinique_filtered['smoking_status'].replace({'ex': 2, 'current': 3})

#Mettre les packyears en classes
# Définir les intervalles et les labels correspondants
bins = [0, 10, 30, float('inf')]
labels = [1, 2, 3]
# Utiliser pd.cut pour transformer les valeurs continues en valeurs discrètes
df_clinique_filtered.loc[:, 'packyears'] = pd.cut(df_clinique_filtered.loc[:, 'packyears'], bins=bins, labels=labels, right=False)

# Fusionner les deux DataFrames
df_combined = pd.concat([df_genes_AEGIS, df_clinique_filtered], axis=1)


## 2- tp3

# Changer la colonne cancer par des 0 ou 1
print(df_clinique_AEGIS['cancer_status'])
df_clinique_AEGIS['cancer_status'] = df_clinique_AEGIS['cancer_status'].replace({'Cancer':1, 'NoCancer':0})
print(df_clinique_AEGIS['cancer_status'])

# Extraire les descripteurs et l'étiquette
X = df_combined
y = df_clinique_AEGIS['cancer_status']
...
## ex : ([1, 2, 3, 4], [5]), ([1, 2, 3, 5], [4])... et on les test tous (5)
    ...
    logistique1 = LogisticRegression(penalty='elasticnet', solver='saga', max_iter=10000)
    ...
    logistique2 = LogisticRegression(penalty='elasticnet', solver='saga', C=best_C, l1_ratio=0.5, max_iter=9000)
...
plt.title('Histogramme de la sélection des coefficients AEGIS')
plt.show()
